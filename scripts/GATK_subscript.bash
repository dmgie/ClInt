...
GENOME=$1 # The human genome reference
>>>>>>> temp-improve
...
=======
# Read the value of rna_categories["Assembly"] and turn the string into a list by splitting the string by the delimiter ";"
# Then, loop through the list and run the GATK pipeline for each assembly
IFS='§' read -ra ASSEMBLIES <<< "${rna_categories["Assembly"]}"
for assembly_program in "${ASSEMBLIES[@]}"; do
    # check if the assembly program value is 1 in rna_programs assosciative array
    if [ ${rna_programs[$assembly_program]} -eq 1 ]; then
        # Run the GATK pipeline
        echo "Running GATK pipeline for ${assembly_program}"
